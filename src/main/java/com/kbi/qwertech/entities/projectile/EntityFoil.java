...
public class EntityFoil extends EntityRock {
	...
	public EntityFoil(World world) {
		super(world);
	}
	
	public EntityFoil(World world, EntityLivingBase eb2)
	{
		super(world, eb2);
	}
	
	public EntityFoil(World p_i1775_1_, double p_i1775_2_, double p_i1775_4_, double p_i1775_6_)
    {
        super(p_i1775_1_, p_i1775_2_, p_i1775_4_, p_i1775_6_);
    }
	
	 public EntityFoil(World p_i1755_1_, EntityLivingBase p_i1755_2_, float p_i1755_4_, OreDictMaterial mt, short rockType)
	    {
				...
				if (!this.worldObj.isRemote && ((Math.abs(this.motionX) + Math.abs(this.motionY) + Math.abs(this.motionZ)) < 0.1)) {
					...
					if (this.getMaterial().contains(TD.Properties.EXPLOSIVE) && rand.nextFloat() > 0.5)
					{
						...
						if (this.objectType == 2) {
							this.entityDropItem(OP.foil.mat(this.getMaterial(), 1), 1);
						} else {
							//haven't added anything else crumply
						}
					}
					this.setDead();
				}
